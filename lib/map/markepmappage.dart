...
class _MarkerMapPageState extends State<MarkerMapPage> {
            ...
            child: ElevatedButton(
              onPressed: () {
                if (_selectedLocationDetails != null) {
                  widget.onLocationSelected(_selectedLocationDetails!);
                  Navigator.pop(context);
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
  ...
  Future<void> _zoomToLocation(String location) async {
    try {
      List<Location> locations = await locationFromAddress(location);
  ...
  Future<void> _addMarkerFromTap(LatLng position) async {
    _selectedLocation = position;
    ...
    if (placemarks.isNotEmpty) {
      ...
      print('No address details available');
    }
  }
      ...
      children: [
        ...
        Checkbox(
          ...
          onChanged: (value) {
            ...
            widget.onToggleAutoZoom(_autoZoomEnabled);
          },
        ),
        Text('Auto Zoom'),
      ],
